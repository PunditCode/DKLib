...
/* Usage example
    ...
    const CString &str = EnumString<WeekEnd>::From( Saturday );
...
#define Enum_String(EnumeratorName)                                             \
            RegisterEnumerator( EnumeratorName, _T(#EnumeratorName) );
//      }
...
// Types
protected:
    typedef std::map<CString, EnumType> AssocMap;
...
protected:
    ...
    static void RegisterEnumerator(const EnumType e, const CString &eStr);

public:
    // Converts from an enumerator to a string.
    // Returns an empty string if the enumerator was not registered.
    static const CString &From(const EnumType e);

    // Converts from a string to an enumerator.
    // Returns true if the conversion is successful; false otherwise.
    static const bool To(EnumType &e, const CString &str);
};
...
template <class D, class E>
void EnumStringBase<D,E>::RegisterEnumerator(const E e, const CString &eStr)
{
...
template <class D, class E>
const CString &EnumStringBase<D,E>::From(const E e)
{
    for(;;) // Code block
    {
...
template <class D, class E>
const bool EnumStringBase<D,E>::To(E &e, const CString &str)
{
